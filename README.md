# 📝 TODO-Chatbot — To-Do Manager with Telegram Bot

## 📖 Описание проекта

**Todo-Chatbot** — это система управления задачами, которая объединяет веб-интерфейс и Telegram-бота. Пользователи могут создавать, редактировать и отслеживать задачи как через браузер, так и прямо из Telegram. Веб-панель отображает аналитику в реальном времени: количество пользователей, общее число задач, выполненные и незавершённые. Telegram-бот позволяет управлять задачами через команды — устанавливать приоритеты, дедлайны, искать и отмечать задачи выполненными.
